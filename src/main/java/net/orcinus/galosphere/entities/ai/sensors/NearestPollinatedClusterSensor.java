    ...
    protected void doTick(ServerLevel world, Sparkle entity) {
        ...
        if (!poses.isEmpty()) {
            ...
            for (BlockPos pos : poses) {
                if (!entity.getBrain().hasMemoryValue(GMemoryModuleTypes.NEAREST_POLLINATED_CLUSTER)) {
                    entity.getBrain().setMemory(GMemoryModuleTypes.NEAREST_POLLINATED_CLUSTER, pos);
                }
            }
        } else {

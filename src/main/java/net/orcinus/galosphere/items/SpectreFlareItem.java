...
public class SpectreFlareItem extends Item implements ProjectileItem {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        ...
        if (!level.isClientSide) {
            SpectreFlare spectreFlare = new SpectreFlare(level, player, itemStack);
    ...
    public Projectile asProjectile(Level level, Position position, ItemStack itemStack, Direction direction) {
        return new SpectreFlare(level, itemStack.copyWithCount(1), null, position.x(), position.y(), position.z(), false);
    }
}